...
func HandleHTTPErrorNew(resp *http.Response) error {
	// Read the body of the response (if any)
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return fmt.Errorf("failed to read error response: %w", err)
	}

	// Construct an error message based on the status code
	switch resp.StatusCode {
	case http.StatusUnauthorized:
		return fmt.Errorf("unauthorized access: %s", body)
	case http.StatusForbidden:
		return fmt.Errorf("access forbidden: %s", body)
	case http.StatusNotFound:
		return fmt.Errorf("resource not found: %s", body)
